...
java "-javaagent:$(pwd)/ext/jacocoagent.jar=output=tcpserver" -jar target/server.jar &
SERVER_PID=$!
sleep 10
...
echo "@@@ FIRST_RUN"
RUN=$(java -jar target/tests.jar |tee /dev/fd/5)
...
echo "@@@ SECOND_RUN"
RUN=$(java -jar target/tests.jar --optimize-coverage=100% |tee /dev/fd/5)
echo ${RUN} | grep -q "3 passed"
